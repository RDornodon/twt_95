...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "to_submit_ch_95.py"
    ...
    OTHER_LANG_COMMAND = "python3 twt_95.py"
    ...
    TEST_CASE_FILE = "test_cases.json"         #  1000 cases
    TEST_CASE_FILE = "test_cases_a.json"  # 75000 cases
...
def print_extra_stats(
    ...
    print(
        f" - Max prime: {yellow}"
        f"{max(int(x) for x in test_out):_}{reset}"
    )
    print(
        f" - Average prime: {yellow}"
...
def print_speed_summary(speed_num_cases: int, loops: int, times: List[float]) -> None:
    ...
    print(
        f" -{emojis.leopard} Average loop time from top 5 fastest: "
        f"{yellow}{sum(times[:5])/5:.4f}{reset} seconds / {cyan}{speed_num_cases}{reset} cases."
    )
    print(
